...
class PaymentConfirmation extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        // check whether Advance payment is approved 
        $isAdvanceApproved = optional($reservation->payments()->where('payment_alias', 'Preliminary')->first())->status == 2;

        $mail = (new MailMessage)
            ->subject($paymentTypeLabel . ' Submitted for Review - Reservation #' . ($reservation->ref_code ?? $reservation->id))
            ->greeting('Dear Admin,')
            ->line('The customer has submitted the **' . $paymentTypeLabel . '**. Please take necessary action.')
            ->line('---')
        ...
        {
            // if adnvance payement is approved and balance payment is pending approval
            $mail
                ->line('Advance Payment: **Rs. ' . number_format($reservation->advanceAmount, 2) . '**' . '-' . 'Approved')
                ->line('Balance Amount: **Rs. ' . number_format($remaining, 2) . '**' . '(Submitted on : ' . ($payment?->created_at ? \Carbon\Carbon::parse($payment->created_at)->format('d M Y, h:i A') : 'N/A') . ')')
                ->line('---')
        ...
        {
            // if advance payment is pending approval
            $mail
