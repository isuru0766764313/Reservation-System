...
<body>
  ...
  <div class="container-fluid mt-4">
    ...
    <div class="dashboard-section">
      <div class="section-header">
        <h4 style="color: #1b10e6ff;"><i class="fas fa-history me-2" style="color: #1b10e6ff;"></i>Reservation Requests
        </h4>
  ...
  $remainingAmount = max(0, (($reservation->charge - ($reservation->discount_custom ?? 0)) + $reservation->deposit) - $totalPaid);
              ...
              <tr>
                ...
                <td class="align-middle">{{ date('h.i A', strtotime($reservation->start_time)) }} To
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content border-0 shadow-lg">
        ...
        <div class="modal-body">
          ...
          <p class="mb-1">This hall has <strong id="ongoingReservationsCount">0</strong> ongoing reservation(s) that
            have not passed yet.</p>
          <p class="text-muted">A hall with ongoing reservations cannot be made unavailable. Please wait until all
            reservation dates have passed.</p>
          <hr>
                                        ...
                                        <div class="col-md-6">
                                          <dl class="row">
                                            ...
                                            @if ($reservation->status == 1)
                                                ...
                                                <dd class="col-sm-8">
                                                  <input type="date" class="form-control form-control-sm" id="cancellationExpiryDate-{{ $reservation->id }}"
                                                    name="cancellationExpiryDate" min="{{ now()->format('Y-m-d') }}"
                                                ...
                                                <dd class="col-sm-8">
                                                  <div class="input-group input-group-sm">
                                                    <input type="number" class="form-control form-control-sm" step="0.1" min="0" max="100"
                                                      id="discount-custom-{{ $reservation->id }}" placeholder="0" data-charge="{{ $reservation->charge }}"
                                      ...
                                      @if($reservation->status == 1)
                                        <div class="me-auto">
                                          <form method="POST" action="{{route('admin.reservations.accept', $reservation->id)}}"
                                            ...
                                            <input type="hidden" name="cancellationExpiryDate" id="cancellationExpiryDate-hidden-{{ $reservation->id }}"
